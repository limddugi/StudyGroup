...
import io.lcalmsky.app.modules.zone.application.ZoneService;
...
public class StudySettingsController {
    ...
    public String updateStudy(@CurrentUser Account account, @PathVariable String path, @Valid StudyDescriptionForm studyDescriptionForm,
        ...
        if (errors.hasErrors()) {
            model.addAttribute(account);
            model.addAttribute(study);
            return "study/seeings/description";
        }
    ...
    public void addZones(@CurrentUser Account account, @PathVariable String path, @RequestBody ZoneForm zoneForm) {
        ...
        Zone zone = zoneService.findOrCreateNew(zoneForm.getCityName(), zoneForm.getProvinceName(), zoneForm.getLocalNameOfCity());
    ...
    @ResponseStatus(HttpStatus.OK)
    public void removeZone(@CurrentUser Account account, @PathVariable String path, @RequestBody ZoneForm zoneForm) {
        Study study = studyService.getStudyToUpdateZone(account, path);
    ...
    @GetMapping("/study") // 스터디 설정 내 메뉴 중 스터디 메뉴에 진입했을 때 보여줄 뷰로 이동
    ...
    @PostMapping("/study/publish") // 스터디 공개 버튼과 연동
    ...
    @PostMapping("/study/close") // 스터디 종료 버튼과 연동
    ...
    @PostMapping("/recruit/start") // 팀원 모집 시작 버튼과 연동
    ...
    @PostMapping("/recruit/stop") // 팀원 모집 중단 버튼과 연동
    ...
    @PostMapping("/study/path") // 스터디 경로 수정 버튼과 연동
    ...
    @PostMapping("/study/title") // 스터디 이름 수정 버튼과 연동
    public String updateStudyTitle(@CurrentUser Account account, @PathVariable String path, String newTitle, Model model, RedirectAttributes attributes) {
        ...
        attributes.addFlashAttribute("message", "스터디 이름을 다시 수정했습니다");
        return "redirect:/study/" + study.getEncodedPath() + "/settings/study";
    }

    @PostMapping("/study/remove") // 스터디 삭제 버튼과 연동
